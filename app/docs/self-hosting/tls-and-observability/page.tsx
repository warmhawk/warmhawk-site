  ...
  return (
    <div className="py-16">
      ...
      <p className="text-lg leading-relaxed text-ink-muted max-w-2xl mb-8">
        ...
        ships with real health monitoring and distributed-tracing export already wired in, not left
        as homework.
      </p>
      <AnswerBlock>
        ...
        by default, and native OTEL export is wired in but inert until you point it at a backend you
        run.
      </AnswerBlock>
...
./scripts/install.sh --retry-tls`}
  ...
  --skip-certbot --cert-path /path/to/fullchain.pem --key-path /path/to/privkey.pem`}
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mt-4 mb-10">
        In this mode you own renewal too &mdash; the bundled renewal loop only manages certificates
        it issued itself via certbot.
      </p>
...
echo | openssl s_client -connect app.yourcompany.com:443 -servername app.yourcompany.com 2>/dev/null \\
  | openssl x509 -noout -dates`}
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mb-4">
        Every WarmHawk instance on Tier 1/2 bundles two observability pieces at no extra cost, both
        running on the same internal-only network as everything else:
      </p>
      <ul className="list-disc pl-6 space-y-3 text-[15px] leading-relaxed text-ink-muted max-w-2xl mb-10">
        ...
        <li>
          ...
          Unset, it&rsquo;s inert &mdash; nothing is exported anywhere, and no third-party telemetry
          backend is bundled or required. Point it at a backend you already run or subscribe to
          (Grafana Tempo, Honeycomb, anything OTLP-compatible) to get real traces:
        </li>
      </ul>
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mt-4 mb-14">
        Neither piece phones home to WarmHawk &mdash; both are entirely yours to view, configure, or
        point elsewhere.
      </p>
      ...
      <FaqSection
        items={faqItems}
        title="TLS & observability: questions worth answering up front"
      />
    </div>
  );
}
