...
const services: { name: string; role: string }[] = [
  {
    name: 'nginx',
    role: 'The only container that publishes a host port (80/443). TLS termination, reverse proxy to api.',
  },
  {
    name: 'api',
    role: 'Fastify — the /v1 public API, plus internal-only /internal/* routes for n8n callbacks.',
  },
  {
    name: 'worker',
    role: 'BullMQ worker — picks up dispatch jobs from the queue and actually sends.',
  },
  {
    name: 'migrate',
    role: 'Runs Prisma migrations once on startup, then exits — api/worker wait for it to complete successfully.',
  },
  {
    name: 'postgres',
    role: 'Source of truth: leads, campaigns, mailboxes, domains, execution logs, replies.',
  },
  {
    name: 'redis',
    role: 'Backs the BullMQ dispatch queue — AOF-durable, not just a cache. See Backups & Redis durability.',
  },
  {
    name: 'n8n',
    role: 'Runs the dispatch and reply-poll workflows, calling api’s internal-only routes exclusively.',
  },
  {
    name: 'uptime-kuma',
    role: 'Bundled health/uptime monitoring for every service, on by default, free.',
  },
  {
    name: 'certbot',
    role: 'Certificate renewal loop (certbot renew every 12h) — see TLS & observability for the initial-issuance step.',
  },
];
...
export default function ArchitecturePage() {
  return (
    <div className="py-16">
      ...
      <AnswerBlock>
        A WarmHawk instance runs nine docker compose services: nginx (the only one with a published
        port), the api and worker (Fastify + BullMQ, split so a slow send never blocks the API),
        postgres and redis (source of truth and the durable send queue), migrate (runs once), n8n
        (dispatch/reply-poll workflows), uptime-kuma (bundled monitoring), and certbot (renewal
        loop). Nothing but nginx is reachable from outside your server.
      </AnswerBlock>
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mb-10">
        Only <code className="font-mono">nginx</code> publishes host ports (
        <code className="font-mono">80</code>/<code className="font-mono">443</code>). Every other
        service sits on an <code className="font-mono">internal</code>-only Docker network with no
        published port at all &mdash; there is no nginx location block for{' '}
      ...
      <h2 className="font-display text-2xl font-semibold mb-4">
        Reading logs when something won&rsquo;t come up
      </h2>
      <CodeBlock label="See every service's current state">{`docker compose ps`}</CodeBlock>
...
docker compose logs -f worker`}
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mt-4 mb-10">
        A container in a restart loop: follow logs live rather than tailing once &mdash;
        you&rsquo;ll catch the actual error right before each restart, not just the generic exit
        message.
      </p>
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mb-4">
        Docker Compose merges multiple <code className="font-mono">-f</code> files together, and for
        the <code className="font-mono">ports:</code> key specifically, that merge is a{' '}
...
HTTPS_PORT=8443`}
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mt-4 mb-10">
        Already stuck with duplicate bindings?{' '}
        <code className="font-mono">docker compose config</code> prints the fully-merged effective
        config so you can see exactly what&rsquo;s bound before debugging further.
      </p>

      <h2 className="font-display text-2xl font-semibold mb-4">Resource limits</h2>
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mb-4">
        Every service carries <code className="font-mono">cpus</code>/
        <code className="font-mono">memory</code> limits (and reservations) sized for a modest
        single box &mdash; e.g. <code className="font-mono">api</code>: 1.0 CPU / 512M,{' '}
        <code className="font-mono">worker</code>: 0.5 CPU / 256M,{' '}
        ...
        isn&rsquo;t subject to the <code className="font-mono">ports:</code> concatenation behavior
        above.
      </p>
...
# warmhawk-redis      Up 2 hours (healthy)`}
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mt-4 mb-14">
        A service stuck at <code className="font-mono">(unhealthy)</code> rather than crash-looping
        is usually still running but failing its own internal check &mdash; check that
        service&rsquo;s logs specifically, since the process itself hasn&rsquo;t necessarily exited.
        For the bundled uptime/alerting layer watching these same health checks continuously, see{' '}
