  ...
  return (
    <div className="py-16">
      ...
      <AnswerBlock>
        ...
        customFields object, and DELETE /v1/leads/erase handles GDPR right-to-erasure by anonymizing
        PII while preserving aggregate counts.
      </AnswerBlock>
  ...
  -d '{
    ...
    "customFields": { "jobTitle": "VP Marketing", "recentNews": "closed a $12M Series A" }
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mb-4">
        ...
        <code className="font-mono">email</code>/<code className="font-mono">firstName</code>/
        <code className="font-mono">lastName</code>/<code className="font-mono">company</code>{' '}
        (case-insensitive) folds automatically into <code className="font-mono">customFields</code>:
      </p>
...
jane@prospect-co.com,Jane,Doe,Prospect Co,VP Marketing,closed a $12M Series A`}
      ...
      <h2 className="font-display text-2xl font-semibold mb-4">
        Webhook ingest &mdash; unauthenticated by design
      </h2>
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mb-4">
    ...
    "customFields": { "jobTitle": "VP Marketing" }
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mt-4 mb-10">
        ...
        <code className="font-mono">
          200 {'{ status: "skipped", reason: "duplicate" | "suppressed" }'}
        </code>{' '}
        on a skip, or <code className="font-mono">422 {'{ status: "rejected", reason }'}</code> if
  ...
  -d '{ "email": "jane@prospect-co.com" }'`}
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mt-4 mb-10">
        ...
        overwritten with a non-reversible placeholder and{' '}
        <code className="font-mono">piiErasedAt</code> stamped &mdash; the row itself is kept so
        send/reply/campaign aggregate counts referencing it stay accurate. Returns{' '}
        <code className="font-mono">{'{ email, leadsErased }'}</code>, vacuously succeeding with{' '}
        <code className="font-mono">leadsErased: 0</code> if nothing matches. This is distinct from{' '}
        <code className="font-mono">DELETE /v1/leads/:id</code>, a real hard delete of one row by
        id, and from a manual suppress (
      ...
      <div className="border-t border-border pt-10">
        ...
        <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mb-4">
          ...
          straight onto a <code className="font-mono">customFields</code> key using whichever ingest
          path fits: CSV import for a one-time export, or the webhook above for Clay&rsquo;s own
          live enrichment table pushing rows as they finish.
        </p>
        <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mb-4">
          Reference those fields directly in the campaign&rsquo;s{' '}
          <code className="font-mono">aiPromptTemplate</code> (not the plain{' '}
          <code className="font-mono">template</code> field, which has no per-lead substitution)
          with a flat <code className="font-mono">{'{{fieldName}}'}</code> placeholder &mdash; a
          customField named <code className="font-mono">recentNews</code> is{' '}
...
No generic filler, no invented facts.`}
        ...
        <p className="text-[15px] leading-relaxed text-ink-muted max-w-2xl mt-4 mb-4">
          WarmHawk fills whichever placeholders it has data for, then hands the whole lead context
          to your connected Gemini or Claude key (BYOK &mdash; you hold the key, never routed
          through a shared gateway) to write the finished body. See{' '}
          <Link
