  ...
  return (
    <div className="wrap py-16">
      ...
      <p className="text-lg leading-relaxed text-ink-muted max-w-2xl mb-8">
        The mechanics behind SPF, DKIM, DMARC, and the rest of what decides whether a message lands
        in the inbox or the junk folder — written for the person who has to actually fix it, not
        just be told it&rsquo;s broken.
      </p>
      <AnswerBlock>
        This is WarmHawk&rsquo;s blog: short, specific explainers on email authentication and
        deliverability, each one grounded in a real mechanism (an RFC limit, a DNS lookup budget, an
        alignment rule) rather than generic advice. Start with whichever post matches what
      ...
      <div className="mt-14 grid gap-5 max-w-2xl">
        {blogPosts.map((post) => (
          ...
          >
            <div className="text-[11px] font-mono text-ink-muted mb-2">{formatDate(post.date)}</div>
