...
// env file in this repo — deliberately narrower than a layered-env-file approach (e.g. one that
// stacks .env/.env.prod + .env/.env.local on top) that a differently-shaped repo might use instead.
...
// subscription rather than re-running an idempotent check).
//
